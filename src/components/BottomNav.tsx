...
export function BottomNav() {
  ...
  return (
    <nav className="bottom-nav md:hidden">
      {items.map((item) => <NavLink key={item.to} to={item.to} end={item.to === '/'} className={({ isActive }) => `bottom-nav-item ${isActive ? 'bottom-nav-active' : ''}`}><item.icon className="h-5 w-5" /><span>{language === 'ar' ? item.ar : item.en}</span></NavLink>)}
    </nav>
  );
}
