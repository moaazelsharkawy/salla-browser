import { Bookmark, ClipboardList, Download, Languages, LayoutDashboard, LogIn, LogOut, MapPin, Megaphone, Moon, Send, ShieldCheck, Sun, UserPlus, UserRound, X } from 'lucide-react';
  ...
  const modal = <>
    <div className="account-overlay" role="presentation" onMouseDown={onClose}>
      <section className="account-dialog" role="dialog" aria-modal="true" aria-label={ar ? 'حسابي' : 'My account'} onMouseDown={(event) => event.stopPropagation()}>
        <div className="account-dialog-head"><div className="min-w-0"><p className="text-xl font-black">{ar ? 'حسابي' : 'My account'}</p><p className="muted-text mt-1 text-xs font-bold">{ar ? 'إعدادات المتصفح وخيارات المطورين' : 'Browser settings and developer options'}</p></div><button className="icon-button h-10 w-10 shrink-0" onClick={onClose} aria-label={ar ? 'إغلاق' : 'Close'}><X className="h-5 w-5" /></button></div>
        ...
        <div className="developer-panel">
          ...
          {isDeveloper ? <div className="mt-3 space-y-1.5">
            ...
            {isAdmin && <><Link to="/admin" onClick={onClose} className="menu-row"><LayoutDashboard className="h-5 w-5 text-violet-300" /><span>{ar ? 'لوحة الإدارة' : 'Admin dashboard'}</span></Link><Link to="/admin/promotions" onClick={onClose} className="menu-row"><Megaphone className="h-5 w-5 text-amber-400" /><span>{ar ? 'إدارة الترويج والإعلانات' : 'Promotion and ads'}</span></Link></>}
          </div> : <div className="mt-3 grid gap-2 sm:grid-cols-2"><Link to="/login" onClick={onClose} className="primary-button"><LogIn className="h-4 w-4" />{ar ? 'دخول المطورين' : 'Developer sign in'}</Link><Link to="/register" onClick={onClose} className="secondary-button"><UserPlus className="h-4 w-4" />{ar ? 'إنشاء حساب مطور' : 'Create developer account'}</Link></div>}
