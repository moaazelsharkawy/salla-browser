        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="mt-1 line-clamp-2 text-[11px] font-semibold leading-5 text-slate-400 sm:text-xs">
                {language === 'ar' ? app.short_description_ar : app.short_description_en}
              </p>
            </div>
            {onFavorite && (
              <button type="button" aria-label="favorite" onClick={onFavorite} className={`icon-button h-9 w-9 ${favorite ? 'text-rose-300' : 'text-slate-400'}`}>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {app.installable && <span className="inline-flex items-center gap-1 rounded-full bg-white/[0.045] px-2.5 py-1 text-[10px] font-black text-slate-300"><Pin className="h-3 w-3" />{language === 'ar' ? 'قابل للتثبيت' : 'Installable'}</span>}
          </div>
        </div>
      </div>
      <div className="relative mt-4 flex gap-2">
        <Link to={`/apps/${app.slug}`} className="secondary-button flex-1">{language === 'ar' ? 'عرض التفاصيل' : 'Details'}</Link>
        <Link to={`/browse?app=${encodeURIComponent(app.slug)}`} className="primary-button flex-1"><ExternalLink className="h-4 w-4" />{language === 'ar' ? 'فتح التطبيق' : 'Open app'}</Link>
      </div>
    </article>
  );
}
