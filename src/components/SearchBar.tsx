...
export function SearchBar({ value, onChange, compact = false }: { value?: string; onChange?: (value: string) => void; compact?: boolean }) {
  ...
  return (
    <form onSubmit={submit} className={`search-shell ${compact ? 'search-shell-compact' : ''}`}>
      <Search className="search-leading-icon h-5 w-5 shrink-0" strokeWidth={2.5} />
      <input
        value={query}
        onChange={(event) => setQuery(event.target.value)}
        className="search-input min-w-0 flex-1 bg-transparent"
        placeholder={language === 'ar' ? 'ابحث في التطبيقات أو الويب' : 'Search apps or the web'}
        aria-label={language === 'ar' ? 'بحث' : 'Search'}
      />
      {query && (
        <button type="button" className="icon-button h-8 w-8 shrink-0" onClick={() => setQuery('')} aria-label={language === 'ar' ? 'مسح البحث' : 'Clear search'}>
          <X className="h-4 w-4" />
        </button>
      )}
      <button type="submit" className="search-go shrink-0" title={language === 'ar' ? 'بحث على Google' : 'Search Google'}>
