...
-- 2) Publish linked apps that are still drafts.
-- Suspended apps are deliberately left suspended.
...
-- 3) Make the review RPC self-healing for already-approved legacy submissions.
...
begin
  ...
  -- Old versions could leave the submission approved while its app stayed draft
  -- and app_id remained null. Repair that state instead of returning immediately.
  if s.status = 'approved' then
    ...
    -- Extremely defensive recovery: if an approved legacy request has no app
    -- row at all, create the missing published app now.
